...
from models import Employee, CompetenceField, Action, DevelopmentPlanType, ActionComment, Company, Reminder, \
    ReminderTemplate
...
class PasswordResetForm(forms.Form):
    ...
    def clean(self):
        ...
        if not 'email' in self.cleaned_data or not User.objects.filter(
...
class EmployeeForm(forms.Form):
    ...
    last_name = forms.CharField(label=_(u'Last name'), max_length=100,
                                widget=forms.TextInput(attrs={'class': "form-control"}))
    user_name = forms.CharField(label=_(u'Username'), max_length=75,
                                widget=forms.TextInput(attrs={'class': "form-control"}))
...
class AttachMUSForm(forms.Form):
    ...
    competence_fields = forms.ModelMultipleChoiceField(widget=forms.SelectMultiple,
...
class ActionForm(forms.ModelForm):
    ...
    difficulty = forms.ChoiceField(label=_("Difficulty"), choices=Action.getDifficultyChoices(),
                                   widget=forms.Select(attrs={'class': "form-control"}))
    type = forms.ChoiceField(label=_("Type"), choices=Action.getTypeChoices(),
                             widget=forms.Select(attrs={'class': "form-control"}))
