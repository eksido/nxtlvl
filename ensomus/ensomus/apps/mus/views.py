...
def clone_competencefields(request):
	...
	for cf in CompetenceField.objects.filter(is_manager=False,language_code='da',development_plan_type__pk=1,company__isnull=True):
		cf.cloneTo(cddtype)
	for cf in CompetenceField.objects.filter(is_manager=True,language_code='da',development_plan_type__pk=1,company__isnull=True):
		cf.cloneTo(cddtype)
...
def developmentplan_mark_link_as_read(request):
	...
	DevelopmentPlanPageStatus.objects.get_or_create(development_plan_relation=dpr,pagelink=link,status=DevelopmentPlanPageStatus.READ)
...
def development_plan_html(request,development_plan_user_id):
	...
	return TemplateResponse(
			...
			'mus/development_plan_pdf.html',
			{
				...
				'extension' : "%s_%s%s" % (devplan.language_code.lower(),devplan.type.name.lower()[:-3],development_plan_user.isManagers() and "_manager" or ""),
				'development_plan_user': development_plan_user,
...
def development_plan_pdf(request,development_plan_user_id):
	...
	employee = request.user.employee_user.first()
	if (development_plan_user.is_private and development_plan_user.user.pk != request.user.pk) or (not employee.isEnsoUser() and employee.company.pk != development_plan_user.getDevelopmentPlan().owner.company.pk):
		raise PermissionDenied()
	with open(development_plan_user.getPDFfile()) as pdf:
		response = HttpResponse(pdf.read(),content_type="application/pdf")
		response['Content-Disposition'] = 'inline;filename=%s %s.pdf' % ("preparation guide",development_plan_user.user.get_full_name())
		return response
...
def attach_development_plan(request,company_id):
	...
	if employee.isCompanySuperUserOrHigher():
		...
		employeeQS = Employee.objects.filter(
			...
			Q(manager__manager__manager__manager=employee),
			# AND
			Q(newest_development_plan__isnull=True) |
			# OR
			Q(
	...
	if request.method == 'POST':
		...
		form = AttachMUSForm()
		form.fields['employees'].queryset = employeeQS
	cfs = CompetenceField.objects.filter(Q(company=None)|Q(company__pk=company.pk),parent=None,is_system=False).order_by('sort_order')
...
def employee_download_file(request, employee_id, filename):
	...
	response = HttpResponse(
		wrapper,
		# i'd rather do this hack than use urllib.pathname2url
...
def create_employee(request,company_id):
	...
	form.fields['development_plan_type'].queryset = DevelopmentPlanType.objects.filter(Q(company=company)|Q(company__isnull=True))
	return TemplateResponse(
		request,
