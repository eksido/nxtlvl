...
from ensomus.apps.mus.views import employees_json, create_employee, edit_employee, all_employees, \
    attach_development_plan, create_many_employees
from ensomus.apps.mus.views import files, get_file
from ensomus.apps.mus.views import development_plan, get_development_plan, get_development_plan_states, \
    get_development_plan_competence
from ensomus.apps.mus.views import get_development_plan_competence_field, finish_development_plan, \
    development_plan_receipt, development_plan_pdf, development_plan_html, reopen_development_plan
from ensomus.apps.mus.views import save_development_plan_question, remove_development_plan, \
    developmentplan_mark_link_as_read
from ensomus.apps.mus.views import action_list, action_detail, action_add, action_edit
from ensomus.apps.mus.views import dashboard, start_view, change_company, accesscode, password_reset, \
    password_reset_done, clone_competencefields
...
urlpatterns = patterns('',  # Examples:  # url(r'^$', 'ensomus.views.home', name='home'),
                       ...
                       url(r'^developmentplan/(?P<development_plan_id>\d+)/reopen/(?P<is_managers>\w*)$',
                           reopen_development_plan),
                       ...
                       url(r'^developmentplan/save/(?P<development_plan_user_id>\d+)/(?P<question_id>\d+)$',
                           save_development_plan_question),
                       url(r'^developmentplan/get-states/(?P<development_plan_user_id>\d+)$',
                           get_development_plan_states),
                       url(r'^developmentplan/get-competence/(?P<development_plan_user_id>\d+)/(?P<competence_id>\d+)$',
                           get_development_plan_competence),
                       url(
                           r'^developmentplan/get-competence-field/(?P<development_plan_user_id>\d+)/(?P<competence_field_id>\d+)$',
                           get_development_plan_competence_field),
                       url(r'^files/$', files),
